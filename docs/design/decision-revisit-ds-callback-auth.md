...
- **B. 服务端驱动终态 + 持久 release outbox(独立推荐)**：把 terminal-release outbox 行与
...
若批准 B，还须一并决定 Hub quarantine assignment 迁移/UID 退役是否共用持久 outbox，以及
`allocation_uncertain` 的权威 reconciler；在此之前三者保持“安全隔离、可能不可用”，不得声称自动恢复。
