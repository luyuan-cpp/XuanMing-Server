...
> Fleet 回调地址。剩余是 (a) 真 UE 客户端完成 Hub/Battle NetDriver 端到端验收，
> (b) 为 DS→后端补 workload identity（mTLS/ext_authz/短时效 DS token）。
...
挂 grpc_web filter 做协议转换，**按方法白名单（path 精确匹配，deny-by-default）**路由到 4 个内部
服务 cluster（`hub_allocator_cluster` / `ds_allocator_cluster` / `locator_cluster` /
`battle_result_cluster`）。docker-compose 已映射 `8444:8444`。
...
> `BattleResultService/ReportResult`。控制面（Allocate/Release/Transfer/List*/AssignHub 等）、
...
> Allocate/Release/Transfer/List/ClearLocation 等非 DS 回调方法。注意白名单只缩小攻击面，不能认证
> 允许的 `SetLocation` / `ReportResult` / Heartbeat / GM Poll/Ack；生产仍需补 DS workload identity。
> 新增 DS 方法必须显式加一行。
...
> 当前边界是精确 method 白名单 + NetworkPolicy，生产仍须补 mTLS/ext_authz/短时效 DS token 等
> workload identity，并绑定 pod/match。
...
**联调/生产残留（留 Codex/人）**：
- 启动脚本已负责宿主 Envoy force-recreate、集群内 Envoy apply/restart 与 Fleet env 覆盖；需在真 UE
  客户端上验收 7 个回调 method、`TLS=0` 和 Hub/Battle 往返。
- 生产补 DS workload identity（mTLS/ext_authz/短时效 DS token）；不能把 NetworkPolicy/白名单
  写成已完成身份认证，也不可裸露公网。
...
- **UE DS 走 gRPC-Web 而非原生 gRPC**（§5 传输方案偏差）：**§5.1 方案 A 已落地**（:8444 独立 DS 面
  监听器 + 4 个内部服务 cluster）；启动脚本已自动重载 Envoy，并由 Fleet env 覆盖 DS endpoints。
- **玩家身份已上 DSTicket 校验**（2026-06-09）：Hub/Battle DS 在 `PreLogin`/`InitNewPlayer` 强制验
  ...
  dev=HS256 共享密钥，**生产须切 RS256**（DS 只揣公钥）。
