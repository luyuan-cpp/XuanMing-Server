...
- **占位验证**：可先用 `deploy/k8s/agones` 的 simple-game-server 占位 Fleet 验 Agones 分配链路
  （见 README §4 第一步）；心跳 / locator 闭环、战斗结算 → 段位补偿链由真 UE DS（Pandora-Client
  仓库）端到端验证（Heartbeat + SetLocation、同步 ReportResult + GetMatchResult →
...
- Codex/人：UE DS `SetEndpoints` 指向 :8444；经真 UE DS 走一次心跳验 :8444 的 grpc-web 路径走通。
...
中继对局域网开放后暴露 `UDP 7000-8000`（1001 端口）+ Envoy `TCP 8443/8444`，且中继本身：
