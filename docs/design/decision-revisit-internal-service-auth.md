...
> 在人明确选择本文 §9 的方案前，**不得把任一候选描述成已落地或已获批准**；生产 online
> Apply 继续保持阻断。本文不授权安装 mesh、写入 secret、修改生产集群或放宽现有安全门。
...
1. **批准方案 A（推荐）**：将 revision 化 Istio `STRICT mTLS + SPIFFE + AuthorizationPolicy` 定为
   Pandora 内部系统 RPC 的统一最终身份层；Inventory 作为第一条落地链，并与 DS `:8444` 共用基础设施；
2. **选择方案 B**：现阶段不引入 mesh，改为独立 per-service RS256 JWT；接受它不提供链路机密性和
   服务端身份、需要五套密钥轮换、且未来仍须另行交付 mTLS 的成本。

未得到明确选择前，不实施任一方案，不以共享 secret、IP/NetworkPolicy、来源 header、`callerID == 0`
或“只把 `EnsureAuctionEscrow` 藏在 Envoy 后面”作为临时替代。
