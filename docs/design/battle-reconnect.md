...
> 2026-07-12 对抗复核：原文“不会烂数据”只证明 locator BATTLE 值不被后置通知覆盖，不能撤销此前
> 已产生的 Hub seat/assignment/ticket；Hub admission 也尚未核 placement。故查询全失败回 Hub 已上调为
> `decision-revisit-ds-callback-auth.md §7.16.3` 的 production blocker。人拍板前保留既有可用性行为，
> 不再把“下次自愈/Hub 入口对账”描述成安全闭环。
...
2. locator 的 BATTLE fence 会拒绝后置 `LOGIN_PENDING/HUB` 覆盖，Hub join 也可尝试对账；但这些动作
   发生在 seat/assignment/ticket 生成之后，且对账自身可能故障，所以只能缩小窗口，不能证明一人一 DS。
3. 真正闭环必须由 §7.16.3 的“未知即拒绝”或版本化 placement lease + Hub admission 最终门完成。
...
- **§1 一人一 DS**:locator 成功返回 `InBattle` 的路径已 fail-closed，且 BATTLE→BATTLE 只接受同
  match 续期；locator 查询不可判定的路径仍是 §7.16.3 production blocker，本节不得再宣称完整满足。
...
## 7. 全链路断线/切后台审计:任意时间点掉线会不会卡死(2026-07-14)

> 审计问题:玩家在「登录 → 选角 → 进 Hub DS」或「匹配 → READY → 进 Battle DS」的
> **任意时间点**切后台或断线,回来之后会不会卡死?还能不能继续切进 DS?
> 结论:**不会卡死;每个断点都有明确恢复路径,最终都能重新进 DS**(客户端代码
> `MyAccountModel` / `MyMatchModel`,服务端 login/matchmaker/locator/hub_allocator/ds_allocator
> 逐点核对)。「重新登录」是万能恢复入口:login `tryBattleReconnect` 查 locator,
> BATTLE → 直连回战斗(现签新票),否则走正常大厅链;裸重登绝不会把战斗中玩家顶出去(§5 fence)。

### 7.1 逐断点行为表

| # | 断点 | 断线/切后台后发生什么 | 恢复路径 | 卡死? |
|---|---|---|---|---|
| 1 | Login RPC 在飞 | HTTP 失败 → `HandleLoginComplete` 判失败,`ConnectingTarget` 复位 | 直接重按登录(Login 幂等,同 account 稳定 player_id) | 否 |
| 2 | 选角界面 / SelectRole 在飞 | 失败停留选角界面,`bSelectRoleInFlight` 复位 | 再次确认选角重试;session 失效则回登录 | 否 |
| 3 | ClientTravel 连 Hub DS 握手窗口 | `HandleNetworkFailure` Hub 分支**不限失败类型**复位 `ConnectingTarget`;UE 默认断线流程回默认关卡,任意客户端关卡 `OnWorldBeginPlay` 兜底再复位 | 重新登录 → 正常大厅链 | 否 |
| 4 | 已在大厅(含组队) | UE 侧回登录关卡;DS 侧 `Logout → ReportDisconnect` 缩 locator TTL 至 ~10s → OFFLINE(Lua 原子只缩不延,travel 去 battle 免疫) | 重登 → battle 空 → 选角 → `SelectRole` 新地址新票进大厅 | 否 |
| 5 | 匹配排队中 | hub 心跳停止捎带 player_ids → locator HUB 位置 ≤30s 过期 → matchmaker `livenessSweepOnce`(10s/轮)reap 死票:ticket/claim/queue 全清 + FAILED 推送。**票被扫是设计使然,防幽灵票堵队列** | 重登进大厅后重新发起匹配 | 否(需重排) |
| 6 | 确认期成员掉线 | 成局最终门 `findOfflineMembers`:有离线 → match CAS 翻 FAILED,离线者删票,其余成员 Requeue | 掉线者重登重排;队友自动回队列 | 否 |
| 7 | READY 已下发、尚未 travel | PC 失焦不暂停照常 travel;移动端挂起恢复后:推送(`CurrentMatchId==0` 不拦,重启也能被 READY 推进场)+ 3s 轮询双链路继续,**每次轮询 `refreshBattleTicket` 现签新 jti 票**,旧票过期无妨 | 恢复后自动连 battle;彻底杀进程则重登走 §6.1 分流 | 否 |
| 8 | travel 连 Battle DS 握手被拒(准入租约未开等) | `FMyBattleConnectRetryPolicy` 透明重试(复用同 match 票);耗尽 → UE 默认断线回默认关卡 | 重登 → locator BATTLE → `LoginResponse` 带 battle_ds_addr 直连回 | 否 |
| 9 | 登录驱动的 battle 直连握手失败 | `FallbackToHubViaIssueDSTicket`:`IssueDSTicket("hub")` 拿当前有效地址+新票回大厅;鉴权错再兜底重 Login(路径 B) | 自动回大厅 | 否 |
| 10 | 战斗局内掉线 | LOL 式自动重连(§6.3):指数退避重登,30s 窗口;服务端 BATTLE 位置由 ds_allocator 心跳按 **roster(含掉线者)** 续期,整局有效 | 窗口内连回原局;超窗自动 `IssueDSTicket(hub)` 回大厅+弹「已离开对局」;超窗后手动重登若局仍在照样直连回 | 否 |
| 11 | 任意时刻杀进程/重启客户端 | 无本地态残留问题:全部状态服务端权威 | 重登 → `tryBattleReconnect` 分流(BATTLE 直连 / 否则大厅) | 否 |

### 7.2 防卡死的关键机制(为什么敢下结论)

1. **客户端登录互斥锁不会锁死**:`RequestLogin` 用 `ConnectingTarget` 防重复,但它在
   三处复位——Hub 握手任何失败类型、battle `PendingConnectionFailure`、以及**任意客户端
   关卡 `OnWorldBeginPlay`**(UE 默认断线总会落回某个关卡)→ 不存在「回到登录界面却
   永远拒绝登录」的状态。
2. **RPC 在飞标志不会永久悬挂**(2026-07-14 验证):`bLoginInFlight`/`bSelectRoleInFlight`/
   `IssueDSTicket` 回调等只在 HTTP 完成回调复位,而引擎默认 `HttpConnectionTimeout=30` /
   `HttpActivityTimeout=30`(UE 5.8 BaseEngine.ini,项目未覆盖),断网/黑洞时完成回调
   最迟 ~30s 必触发(失败) → 标志必复位。重连 tick 遇在飞只跳过本次发送并重排退避,
   不阻塞。push stream / Agones watch 显式 `SetTimeout(0)` 是长连接设计使然,不在此列。
3. **票据永不因切后台失效**:READY 阶段每次轮询现签新 jti;登录重连也是现签;
   回大厅统一 `IssueDSTicket(hub)` 拿新地址+新票(旧票 jti 已消费、旧地址可能已回收,
   **两者都不能复用**,这是曾经的真 bug,已按此约定修)。
4. **服务端不留幽灵态**:排队死票被 liveness sweep 扫掉;空 battle 局 `EmptyBattleTimeout`
   (默认 5m)回收;大厅位置 TTL 30s 自然过期;§5 fence 保证裸重登顶不掉 active BATTLE
   (一人一 DS 不因猛重登破坏)。
5. **双链路进场**:匹配 READY 靠 push 主驱动 + 3s 轮询兜底,push stream 断了(已知残余:
   无自动重订)轮询仍能进场,不依赖单点。

### 7.3 已知残余(非卡死,记录备查)

- push stream 断开无自动重订状态机(`OnStreamClosed` 只广播):匹配有轮询兜底不受影响,
  但纯推送消费方(好友在线态等)恢复依赖重登/重进大厅时重新 Subscribe。
- 排队中断线票被扫,回来必须重新排队——体验取舍(防幽灵票占队列),非缺陷。
- 客户端自动重连窗口 30s < 服务端重连窗口(=对局生命周期):超窗自动回大厅是客户端
  主动选择;若局仍在进行,玩家再登录仍会被直连回战斗(§6.1 分流),行为自洽。
- locator 查询失败降级走 hub 的「一人一 DS」最终门仍是 §2.3 / `decision-revisit-ds-callback-auth.md`
  §7.16.3 的 production blocker,与"卡死"无关但同属本链路,勿混淆。
