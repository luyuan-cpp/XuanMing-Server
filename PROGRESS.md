...
- cpp pb 同步 UE 仓库(`Source/Pandora/Generated/Proto/`)交 Codex(本轮含 proto 改动,[proto] 标记)。

### 下一步(养成域后续批次)

- Batch 2:装备 / 天赋树服务(loadout,需引配置表 item/talent,更复杂)。
- Batch 3:背包服务(inventory,大厅态道具使用 / 出售,经济事务防作弊)。
- Batch 4:battle DS 开战前快照契约文档(ds-arch.md / agones-dev.md),进战下发 GetLoadout。

### Codex 需要做的(收尾)

1. `git status` / `git diff --stat` 确认改动范围。
2. player 跑 `go mod tidy` 固化 go.sum(本轮 proto 改动,需同步 cpp pb 到 UE 仓库,[proto])。
3. 等用户「帮我 commit」再提交(建议独立 commit,scope=player)。
...
- `go.work` 加 `use ./pkg/killswitch/etcdkv`。
- 在 `pkg/killswitch/etcdkv` 执行 `go mod tidy`,生成该独立 module 的 `go.sum`。
- `pwsh tools/scripts/proto_gen.ps1 -Cpp` 通过:buf lint OK,Go pb 33 files,CPP pb 20 files。
- 补充「关闭整个服务 / 恢复服务」runbook,明确 file / etcd / Envoy 三种口径。
