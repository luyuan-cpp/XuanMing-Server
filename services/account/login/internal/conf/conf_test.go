...
func TestMatchResumeAuthSecretRequiredAndIndependent(t *testing.T) {
	base := func() Config {
		var cfg Config
		cfg.Defaults()
		cfg.Login.Matchmaker.Addr = "matchmaker:50011"
		cfg.Login.MatchResumeAuthSecret = "login-match-resume-service-auth-0123456789abcdef"
		cfg.Login.MatchResumeAuthAudience = "matchmaker:5v5_ranked"
		return cfg
	}

	cfg := base()
	if err := cfg.Validate(); err != nil {
		t.Fatalf("valid dedicated key: %v", err)
	}
	for name, key := range map[string]string{
		"player-jwt":  cfg.Login.JWT.Secret,
		"ds-callback": cfg.DSAuth.Secret,
		"placement":   "login-match-resume-service-auth-0123456789abcdef",
	} {
		t.Run(name, func(t *testing.T) {
			candidate := base()
			if name == "placement" {
				candidate.Login.PlacementBootstrapProofSecret = key
			}
			candidate.Login.MatchResumeAuthSecret = key
			if err := candidate.Validate(); err == nil {
				t.Fatal("expected trust-domain overlap rejection")
			}
		})
	}

	cfg = base()
	cfg.Login.MatchResumeAuthSecret = "too-short"
	if err := cfg.Validate(); err == nil {
		t.Fatal("expected short service identity key rejection")
	}
}
