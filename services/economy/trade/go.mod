...
//   双方两阶段确认的玩家交易(挂单→买方确认→卖方确认→结算);
//   订单存 Redis(Order proto bytes + WATCH/MULTI/EXEC 乐观锁);
//   结算走 ResourceLedger 原子扣减 + 幂等键 = order_id(不变量 §9.7);
//   每次状态流转把订单快照发 kafka pandora.trade.audit(弱依赖,审计 / 对账)。
//
// 依赖来源:
//   - pkg/        (公共框架,go.work use:redisx / kafkax / snowflake)
//   - proto/      (trade/v1 + common/v1)
//   - Kratos v2.9.2 / go-redis/v9(订单状态机)/ IBM/sarama(trade.audit)
