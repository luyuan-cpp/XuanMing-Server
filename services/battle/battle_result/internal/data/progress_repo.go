...
// Seq 是产生本行的批末事件 seq;幂等键 = progress:{match_id}:{seq}:{player_id}:{kind 名}。
...
// GetProgressWatermark 读一场对局的进度水位。
// 返回 (last_applied_seq, 是否已结算, 水位行是否存在, error)。行不存在 → (0,false,false,nil)。
func (r *MySQLBattleRepo) GetProgressWatermark(ctx context.Context, matchID uint64) (uint64, bool, bool, error) {
	var (
		lastSeq   uint64
		settledMs int64
	)
	err := r.db.QueryRowContext(ctx,
		`SELECT last_applied_seq, settled_at_ms FROM battle_progress_stream WHERE match_id = ? LIMIT 1`,
		matchID,
	...
	if err != nil {
		return 0, false, false, errcode.New(errcode.ErrUnavailable, "query progress watermark match=%d: %v", matchID, err)
	}
	return lastSeq, settledMs > 0, true, nil
}
...
func (r *MySQLBattleRepo) ApplyProgress(ctx context.Context, matchID, expectedSeq, newSeq uint64, rows []ProgressOutboxRecord) error {
...
VALUES (?, ?, 0, 0, ?)`, matchID, newSeq, nowMs); ierr != nil {
			if isDupErr(ierr) {
		...
		res, uerr := tx.ExecContext(ctx,
			`UPDATE battle_progress_stream SET last_applied_seq = ?, updated_at_ms = ?
WHERE match_id = ? AND last_applied_seq = ? AND settled_at_ms = 0`,
			newSeq, nowMs, matchID, expectedSeq)
		if uerr != nil {
...
FROM battle_progress_outbox ORDER BY id ASC LIMIT ?`
