module github.com/luyuancpp/pandora/services/battle/hub_allocator

go 1.26.5

// W4 ⑤ hub_allocator 服务(Pandora 第 9 个 Kratos 业务服,2026-06-06)。
//
// 职责(docs/design/go-services.md §2.12):大厅 DS 分片调度。
//   AssignHub / ReleaseHub / TransferHub / ListHubs / Heartbeat。
//   Hub DS Fleet 常驻 N 个 pod(每个 ≤500 人),玩家进 hub 按 region + 队友 + 最空分片选;
//   W4 ⑤ 阶段用 MockHubFleetProvider(种确定性假分片)+ Redis 分片镜像;
//   真 Agones Fleet 查询接入留后续环境就绪步。AssignHub 自签 hub DSTicket(pkg/auth)。
//
// 依赖来源:
//   - pkg/        (公共框架,go.work use;pkg/auth 签 hub 票据)
//   - proto/      (hub/v1 + common/v1)
//   - Kratos v2.9.2 / go-redis/v9(分片镜像 + 心跳超时 ZSET 扫描)
//   - google/uuid (DSTicket jti)
