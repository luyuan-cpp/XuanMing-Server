...
//   游戏内邮件系统。系统/公会邮件拉取式(channel + watermark 游标,零写扩散),
//   个人邮件写扩散(离线可达)。三类统一收件箱视图,附件领取走 player_mail_claim 幂等。
//   存储 pandora_social 库(MySQL 强依赖)。
//
// 依赖来源:
//   - pkg/    (go.work:mysqlx / snowflake / grpcserver / config / errcode)
//   - proto/  (mail/v1 + common/v1)
