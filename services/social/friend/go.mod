module github.com/luyuancpp/pandora/services/social/friend

go 1.26.4

// friend 服务(Pandora 社交域好友 / 黑名单,2026-06-15)。
//
// 职责(docs/design/go-services.md §2.4):
//   好友请求 / 接受 / 列表 / 拉黑;好友图落 pandora_social 库(MySQL 强依赖);
//   好友请求 / 接受通过 kafka pandora.friend.event → push 推送给接收方(弱依赖);
//   ListFriends 经 player_locator 填在线状态(弱依赖,查不到按离线)。
//
// 依赖来源:
//   - pkg/        (公共框架,go.work use:mysqlx / kafkax / snowflake / grpcclient)
//   - proto/      (friend/v1 + locator/v1 + common/v1)
//   - Kratos v2.9.2 / IBM/sarama(friend.event 生产)/ go-sql-driver/mysql(pandora_social)
