...
//
// 职责(docs/design/go-services.md §2.7):
//   维护队伍状态机(FORMING→READY→MATCHING→IN_BATTLE→DISBANDED),
//   实现 5 人组队 + 邀请 + 准备 + 踢人流程,通过 kafka pandora.team.update
//   广播队伍变更推送给 push 服务。
//
// 依赖来源:
//   - pkg/        (公共框架,go.work use)
//   - proto/      (team/v1 + common/v1)
//   - Kratos v2.9.2 / IBM/sarama(kafka push) / go-redis/v9(队伍状态 WATCH/MULTI/EXEC)
