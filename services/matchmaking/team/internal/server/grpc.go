...
// NewGRPCServer 构造 gRPC server 并注册 TeamService。
//
// 端口由 cfg.Server.Grpc.Addr 决定(默认 :50010)。
...
func NewGRPCServer(cfg *conf.Config, svc *service.TeamService, sessGate sessiongate.Gate) *kgrpc.Server {
	srv := grpcserver.MustNewServer(cfg.Server, pmw.AuthOptional(),
		pmw.SessionCurrent(sessGate, cfg.SessionGate.Require))
	teamv1.RegisterTeamServiceServer(srv, svc)
	return srv
}
