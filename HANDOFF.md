...
### 9.2 Codex 待办(阻塞整模块编译)

1. 跑 `pwsh tools/scripts/proto_gen.ps1` 重生 leaderboard pb(当前 `internal/service` 编译差 3 个新字段 getter,预期内)。
2. 重生后 `go build ./... && go test ./...`(leaderboard 模块)确认全绿;无需 go mod tidy(零新依赖)。
3. cpp pb 同步 UE 仓库(常规 [proto] 流程;客户端展示估算名次须做粗粒度文案,禁止假精度)。
