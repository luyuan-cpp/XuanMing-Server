...
| 决策复议(待拍板) | 2026-06-09 | **提议 UE 客户端改用生成的 protobuf pb,废弃手写 wire codec**(复议 `2026-06-08` UE 仓库行的「客户端零额外依赖」)。论证见 [`docs/design/decision-revisit-ue-proto-codec.md`](./docs/design/decision-revisit-ue-proto-codec.md)。动因:手写 `FPandoraProtoWriter` 手填字段号 = 与 proto 重复的并行结构(违 §5.8),且丢失 protobuf 前后向兼容保证(后端加字段 UE 老端会丢/错位,违 §9 不变量 5 的精神)。提议中间路线:**只链 libprotobuf-lite(`optimize_for=LITE_RUNTIME`)做消息编解码,不链 grpc-cpp**(gRPC-Web 经 Envoy 是 HTTP/1.1,传输层 `FPandoraGrpcWeb`+`FHttpModule` 保留)→ proto 回归唯一真相源 + 兼容性由运行时保证。cpp pb 产物链已存在(`proto/buf.gen.cpp.yaml`→`proto/gen/cpp`)。分工(AGENTS §11.1):ThirdParty libprotobuf-lite 取得/Build.cs/RTTI-异常开关/UE 编译验证 = Codex/人(碰构建环境);`PandoraProto` module + 消息编解码重构 + 顺带拆 `UPandoraBackendSubsystem` SRP(per-domain client)= Claude。**未拍板前不动 UE 码**。 |
