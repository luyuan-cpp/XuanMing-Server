...
| W4 ⑭ | 2026-06-09 | **本地真 Agones + Kafka + MySQL 两段补偿链跑通 + kafkax producer 超时修复**(无新 proto / 无新 errcode)。hub_allocator 接入真 Agones Fleet 发现(commit 278a2a2,`internal/biz/agones_fleet.go`,Mock 保留 fallback)+ `deploy/k8s/agones/` manifests + `docs/design/agones-dev.md`;Codex 真环境验收(minikube v1.30.0 + K8s v1.30.0 + Agones 1.58.0,Fleet pandora-battle 2/2 + pandora-hub 3/3 Ready region=cn,`ds_allocator allocator_mode=agones` / `hub_allocator fleet_mode=agones`,`AllocateBattle`/`AssignHub` 返真实地址如 `192.168.49.2:7929` / `:7136` + 写 `pandora.dev/match-id|map-id|game-mode` label)。两段补偿链(不变量 §4)端到端:NORMAL Elo +16/-16 守恒(`delta_sum=0`)、outbox_published=10、player MMR 写回 1516/1484、幂等(alreadyRecorded=true);ABANDONED 强制 delta 0、MMR 不变 1500、total_battles=0、mmr_history 10 行 delta 0、outbox 清零;第一段 `ds_heartbeat_stub.ps1` 验心跳超时→abandoned→`ds.lifecycle`,第二段 `battle_result_outbox_probe.ps1` 验 ReportResult→事务出箱→`player.update`→段位回写。**kafkax producer 超时修复(commit d3df901)**:`pkg/kafkax/producer.go` 原无条件把 sarama 30s `Net.DialTimeout/ReadTimeout/WriteTimeout` 默认覆盖为配置零值,而 `Validate()` 要求三者 > 0 → 4 个 producer 服务(ds_allocator/battle_result/team/matchmaker)按各自 yaml 省略的字段初始化失败、仅 Warn,弱依赖事件链(ds.lifecycle/player.update outbox/team.update/match.progress)被静默禁用;sarama/mocks 单测绕过 config 构建故未捕获。修为抽 `buildProducerConfig` + `if d > 0` 正值守卫(零值保留 sarama 默认,不改任何 dev yaml),加 2 个回归单测;Codex 真环境确认 `ds_lifecycle_producer_ready` + `player_update_producer_ready` 出现。脚本侧修 `battle_result_outbox_probe.ps1` 的 ABANDONED 判定(proto3 JSON 省略 0 值字段,改为 stats 存在且无非 0 mmrDelta 即通过)。UE Hub/Battle DS 真实业务 Heartbeat + locator HUB/BATTLE fence 上报落地后替换 stub 跑主链路留后续。 |
| 开发开关 | 2026-06-09 | **login 新增开发期免密登录开关 `login.dev_skip_password`**(默认 `false`,无新 proto / 无新 errcode / 无新表)。`true`(⚠️ 仅本机 / 联调,**绝不上生产**):① 跳过 bcrypt 密码校验,任意 `password_hash` 放行;② 账号不存在时自动懒注册(snowflake 生 `player_id` 写 `accounts`,靠 `uk_account` 唯一),同一账号名每次登录拿**同一个稳定 `player_id`**(持久化 MySQL,非临时算),并发建号走 `ErrAlreadyExists`→回查。用途:客户端联调期随便填个账号名就能登录拿对应 player_id,无需独立注册流程/RPC。改动:`conf.LoginConf.DevSkipPassword` + `biz.Login` 免密分支/`ensureAccount` 懒注册 + `NewLoginUsecase` 签名加 `devSkipPassword` + main 透传 + 启动打 `DEV_SKIP_PASSWORD_ENABLED` 警告日志 + `service_ready` 加 `dev_skip_password` 字段;`login-dev.yaml` 设 `dev_skip_password: true`;biz 加 2 单测。⚠️ **绝不能上生产**——否则任意账号名都能登录任意 `player_id`;生产留 `false` 走正常 bcrypt 校验。login 模块 BUILD/VET/TEST=0。 |
| 决策复议(已拍板) | 2026-06-09 | **UE 客户端/DS 改用生成的 protobuf pb,废弃手写 wire codec**(复议并推翻 `2026-06-08` UE 仓库行的「客户端零额外依赖」)。论证见 [`docs/design/decision-revisit-ue-proto-codec.md`](./docs/design/decision-revisit-ue-proto-codec.md)。动因:手写 `FPandoraProtoWriter` 手填字段号 = 与 proto 重复的并行结构(违 §5.8),且丢失 protobuf 前后向兼容(后端加字段 UE 老端会丢/错位,违 §9 不变量 5)。**已拍板选择**:① protobuf **v35.0**(latest);② **源码随 UE(UBT)构建,不用预编译库**(UE Linux DS 用自带 clang+libc++,系统预编译 `.a` ABI 撕裂;UBT 编译自动匹配 + 跨平台跟着编);③ **只链 libprotobuf,不链 grpc-cpp**(gRPC-Web 经 Envoy 是 HTTP/1.1,传输层 `FPandoraGrpcWeb`+`FHttpModule` 保留)。**硬约束**:protobuf v35.0 强依赖 **abseil-cpp**(ThirdParty 要同时纳管 protobuf v35.0 + 对应 abseil tag,版本钉死)+ **gencode 版本必须 == 运行时版本**(buf.gen.cpp.yaml 已把 cpp 插件升 v35.0 + **移除 grpc/cpp 插件**,`*.grpc.pb.*` 引 grpc++ 头 UE 没有会编不过)。分工(AGENTS §11.1):ThirdParty protobuf+abseil v35.0 源码取得/Build.cs/RTTI-异常开关/buf 或 protoc 生成/UE Win64+Linux 编译验证 = Codex/人(碰构建环境);`PandoraProto` module 消息编解码重构(`set_xxx`+`SerializeToArray`/`ParseFromArray` 换手填)+ 顺带拆 `UPandoraBackendSubsystem` SRP(per-domain client)= Claude(代码可并行起草,**合并前必须等 ThirdParty 落地 + UE 编译通过**)。 |
| TLS/发布 | 2026-06-10 | **生产连接 ②(UE FHttpModule → Envoy)TLS 证书策略固化:公网 CA(Let's Encrypt/商业)+ 真实域名,SAN 不写 IP;dev 自签 mkcert 仅用 `[SSL] DebuggingCertificatePath` 叠加公开 dev CA。**生产玩家设备出厂信任公网 CA,零配置握手;dev 的 `SSL_ERROR_SYSCALL` 本质是 UE OpenSSL 不信任 mkcert CA,不能推断生产。落地文件:`deploy/dev-ca/pandora-dev-rootCA.pem` 只提交公开 CA 证书,私钥继续忽略;`tools/scripts/import_dev_ca.ps1` 只改 UE 客户端工程 `Config/` 不碰引擎;`tools/scripts/release_preflight.ps1` 发布前拦截本机 GatewayHost、dev 自动登录、`dev_skip_password`、reflection、自签/IP SAN 证书。 |
| ID 生成 | 2026-06-11 | **拒绝 Redis INCR 发号,也不使用 Redis 租约分配 snowflake nodeID;当前静态 `node.zone_id` 够用,未来 k8s 多副本动态扩缩时用 etcd Lease 分配 nodeID。**Redis INCR 每次取号有网络往返,比本地 CAS snowflake 慢 4~5 个数量级,且 RDB/AOF/主从切换计数回退会导致重复 ID;Redis `SETNX+TTL+看门狗` 不能做可靠 fencing,租约过期但旧进程仍发号会导致 nodeID 双活。etcd 方案仍需 KeepAlive/session monitor,但它只监听原生 lease 状态;KeepAlive 丢失、lease revoke 或 session done 时进程必须停发并主动退出。落点:以后只加 `NewNodeFromEtcd(...)` 工厂,`snowflake.Node` 和 `Generate` 热路径不改。详见 `docs/design/infra.md` §8.1。 |
