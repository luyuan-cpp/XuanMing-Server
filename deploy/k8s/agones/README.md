...
- 现有 etcd 若仍是旧版非 PVC 布署，普通启动和 `-Resume` 都会 fail-closed。要保留现场，必须先停写、
  ...
  把 missing 偷当成 1。不要直接 apply 新 Deployment 覆盖旧 Pod，否则会丢授权基线。
- 全新 profile 只有在精确确认 key missing 时，才允许一次 CAS bootstrap 到 epoch 1；已有 profile 和
  `-Resume` 永远只读验证。现场不需要保留时，可显式 `-Reset` 重建本地开发集群。
